...
def create_bell_curve_chart(df, mean, std_dev):
    ...
    density_at_points = (1 / (std_dev * np.sqrt(2 * np.pi))) * np.exp(-0.5 * ((df['Batting Average'] - mean) / std_dev) ** 2)
    ...
    for i in range(len(df)):
        while True:
            ...
            if 0 <= new_y <= density_at_points[i]:
                points_with_jitter.append((df['Batting Average'][i], new_y))
                break
    ...
    is_high_fund = np.abs(df['Batting Average'] - mean) > 1.5 * std_dev
